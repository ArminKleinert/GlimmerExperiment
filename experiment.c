...
void make_child(ENTITY *par0, ENTITY *par1, ENTITY *child, uint32_t *rand_seed) {
  ...
  int32_t rand_factor = (*rand_seed = xorshift(*rand_seed)) % (CH_OTHER_GROUP+1);
  int32_t group = (rv01 < rv10  && rand_factor) ? par1->group : par0->group;
  
  child->group = group;
  upto(i, NUM_GROUPS) {
    child->affinities[i] = choose32(par0->affinities[i], par1->affinities[i], rand_seed);
    
    if (i == group && OWN_GROUP_AFF_MOD != 0) {
      child->affinities[i] += (*rand_seed = xorshift(*rand_seed)) % OWN_GROUP_AFF_MOD;
    }
  ...
  upto(i, NUM_ATTR) {
    child->attr_prio[i] = choose32(par0->attr_prio[i] , par1->attr_prio[i], rand_seed);
    child->attr_prio[i] = maybe_flip_bit(child->attr_prio[i], rand_seed);
  }
  
  upto(i, NUM_ATTR) {
    child->attr_vals[i] = choose32(par0->attr_vals[i] , par1->attr_vals[i], rand_seed);
...
int main(int argc, char **argv) {
  ...
  upto (i, NUM_ENTITIES) {
    ...
    randomize_arr(&rand_seed, NUM_ATTR, MIN_ATTR_PRIO, MAX_ATTR_PRIO, e->attr_prio);
    randomize_arr(&rand_seed, NUM_ATTR, MIN_ATTR_VAL, MAX_ATTR_VAL, e->attr_vals);
  }
  ...
  upto (gen, GENERATIONS) {
    ...
    upto (inner_i, NUM_ENTITIES) {
      ...
      rand_seed = xorshift(rand_seed);
      if (g > best_score || (CH_PREFER_NEW && (rand_seed % CH_PREFER_NEW == 0))) {
        best_score = g;
  ...
  finalize_generation();
  //print_all_entities();
  if (DO_SHUFFLE) shuffle_entities(&rand_seed);
  }
